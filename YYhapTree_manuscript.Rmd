...
```{r bb4-dend, echo=FALSE, fig.cap="Dendrogram of haplogroup bb4 (Q - South American)", fig.width=10, fig.height=6}
if("bb4" %in% names(HapTable) && !is.null(HapTable[["bb4"]])) {
  ...
  if(nrow(h) >= 2) {
    ...
    plot(dend, main = "Haplogroup bb4 (Q)", cex=.7, ylab="Number of differing SNPs")
    legend("topright", inset = c(-0.2, 0), legend = group_levels, fill = colors[1:length(group_levels)], title = "Population", cex=.7, bty="n")
...
```{r baaa4aa1-dend, echo=FALSE, fig.cap="Dendrogram of haplogroup baaa4aa1 (R1b - Western European)", fig.width=10, fig.height=6}
if("baaa4aa1" %in% names(HapTable) && !is.null(HapTable[["baaa4aa1"]])) {
  ...
  if(nrow(h) >= 2) {
    ...
    labels_colors(dend) <- label_colors[order.dendrogram(dend)]
    par(mar = c(5, 4, 4, 10), xpd = TRUE)
    plot(dend, main = "Haplogroup baaa4aa1 (R1b)", cex=.7, ylab="Number of differing SNPs")
    legend("topright", inset = c(-0.2, 0), legend = group_levels, fill = colors[1:length(group_levels)], title = "Population", cex=.7, bty="n")
    par(mar = c(5, 4, 4, 2) + 0.1, xpd = FALSE)
  }
}
