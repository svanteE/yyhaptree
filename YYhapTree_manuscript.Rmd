...
# Define helper function for vertex population tables
inVertex=function(x,m3){
  h=x$inHg
  if(length(h)==0) return(data.frame(Message="No profiles"))
  g=m3[h,8009:8010]  # Columns after 8008 SNPs: ori and HG
  colnames(g) <- c("Population", "ISOGG Haplogroup")
  g$Count=1
  result <- aggregate(Count~., g, sum)
...
Recent advances in high-throughput sequencing have enabled comprehensive Y-chromosome variant catalogs from projects such as the 1000 Genomes Project [@1000genomes2015global; @wei2013whole], providing opportunities for novel analytical frameworks. The development of calibrated Y-chromosome phylogenies based on whole-genome resequencing [@hallast2015great; @batini2015large] has greatly expanded our understanding of Y-chromosome diversity and demographic history. The absence of recombination implies that SNP combinations should exhibit perfect linkage, with variants inherited as immutable haplotype blocks except for new mutations. This property suggests that systematic analysis of haplotype structure could reveal phylogenetic relationships without requiring a priori marker selection.
...
Branch **d** contains 8 individuals differing from the ancestral state by only one yyhap (LODY=22). These individuals are identical across all 8,008 SNPs except for 5 positions:
...
Branch **b** (LODY=104) encompasses 371 individuals primarily from Eurasia. Figure 4 shows the detailed structure of this branch:
...
**Example Terminal Haplogroups:**

Terminal haplogroup **bb4** (haplogroup Q) shows South American representation:

```{r bb4-table, echo=FALSE}
if("bb4" %in% names(profs) && !is.null(profs[["bb4"]])) {
  hg_table <- as.data.frame(table(profs[["bb4"]]$HG))
  colnames(hg_table) <- c("ISOGG Haplogroup", "Count")
  kable(hg_table, caption="Haplogroup bb4 ISOGG classification")
}
```

```{r bb4-dend, echo=FALSE, fig.cap="Dendrogram of haplogroup bb4 (Q - South American)", fig.width=8, fig.height=5, out.width="100%", fig.pos="H"}
if("bb4" %in% names(HapTable) && !is.null(HapTable[["bb4"]])) {
  h=HapTable[["bb4"]]
  if(nrow(h) >= 2) {
    h=h[order(h$ori),]
    L=dim(h)[2]
    ...
    plot(dend, main = "Haplogroup bb4 (Q)", cex=.6, ylab="Number of differing SNPs", horiz=FALSE)
    par(xpd = TRUE)
    legend("topright", inset = c(0, -0.15), legend = group_levels, fill = colors[1:length(group_levels)], title = "Population", cex=.6, bty="n", ncol=min(3, length(group_levels)))
    par(mar = c(5, 4, 4, 2) + 0.1, xpd = FALSE)
  }
}
```

Terminal haplogroup **baaa4aa1** shows Western European concentration:

```{r baaa4aa1-table, echo=FALSE}
if("baaa4aa1" %in% names(profs) && !is.null(profs[["baaa4aa1"]])) {
  hg_table <- as.data.frame(table(profs[["baaa4aa1"]]$HG))
  colnames(hg_table) <- c("ISOGG Haplogroup", "Count")
  kable(hg_table, caption="Haplogroup baaa4aa1 ISOGG classification")
}
```

```{r baaa4aa1-dend, echo=FALSE, fig.cap="Dendrogram of haplogroup baaa4aa1 (R1b - Western European)", fig.width=8, fig.height=5, out.width="100%", fig.pos="H"}
if("baaa4aa1" %in% names(HapTable) && !is.null(HapTable[["baaa4aa1"]])) {
  h=HapTable[["baaa4aa1"]]
  if(nrow(h) >= 2) {
    h=h[order(h$ori),]
    L=dim(h)[2]
    ...
    labels_colors(dend) <- label_colors[order.dendrogram(dend)]
    par(mar = c(4, 4, 3, 2), xpd = FALSE)
    plot(dend, main = "Haplogroup baaa4aa1 (R1b)", cex=.6, ylab="Number of differing SNPs", horiz=FALSE)
    par(xpd = TRUE)
    legend("topright", inset = c(0, -0.15), legend = group_levels, fill = colors[1:length(group_levels)], title = "Population", cex=.6, bty="n", ncol=min(3, length(group_levels)))
    par(mar = c(5, 4, 4, 2) + 0.1, xpd = FALSE)
  }
...
**Example Terminal Haplogroup:**

Terminal haplogroup **c1b3** (haplogroup N) demonstrates the Finnish-East Asian connection:

```{r c1b3-table, echo=FALSE}
if("c1b3" %in% names(profs) && !is.null(profs[["c1b3"]])) {
  hg_table <- as.data.frame(table(profs[["c1b3"]]$HG))
  colnames(hg_table) <- c("ISOGG Haplogroup", "Count")
  kable(hg_table, caption="Haplogroup c1b3 ISOGG classification")
}
```

```{r c1b3-dend, echo=FALSE, fig.cap="Dendrogram of haplogroup c1b3 (N - Finnish/Uralic)", fig.width=8, fig.height=5, out.width="100%", fig.pos="H"}
if("c1b3" %in% names(HapTable) && !is.null(HapTable[["c1b3"]])) {
  h=HapTable[["c1b3"]]
  if(nrow(h) >= 2) {
    h=h[order(h$ori),]
    L=dim(h)[2]
    ...
    dend <- as.dendrogram(cl)
    dend=dend%>%set("labels_cex",.7)
    dend=set_labels(dend,labels=profs[["c1b3"]]$HG)
    grouping_factor <- factor(as.character(h$ori))
    group_levels <- levels(grouping_factor)
    colors <- brewer.pal(max(3, length(group_levels)), "Set1")
    label_colors <- colors[as.numeric(grouping_factor)]
    labels_colors(dend) <- label_colors[order.dendrogram(dend)]
    par(mar = c(4, 4, 3, 2), xpd = FALSE)
    plot(dend, main = "Haplogroup c1b3 (N)", cex=.6, ylab="Number of differing SNPs", horiz=FALSE)
    par(xpd = TRUE)
    legend("topright", inset = c(0, -0.15), legend = group_levels, fill = colors[1:length(group_levels)], title = "Population", cex=.6, bty="n", ncol=min(3, length(group_levels)))
    par(mar = c(5, 4, 4, 2) + 0.1, xpd = FALSE)
  }
...
One extreme example is haplogroup **aaa1aaa1aaa1aaa3a1** with 16 Kenyan individuals defined by 20 nested yyhaps, representing one of the most derived lineages in our tree. Remark that this haplogroup is a node in the branch **aaa1aa**, which is not displayed. The ISOGG haplotype is E1b1a1a1f1a1d for all 16 profiles.

**Example Terminal Haplogroups:**

Terminal haplogroup **abaaa3a** shows African representation:

```{r abaaa3a-table, echo=FALSE}
if("abaaa3a" %in% names(profs) && !is.null(profs[["abaaa3a"]])) {
  hg_table <- as.data.frame(table(profs[["abaaa3a"]]$HG))
  colnames(hg_table) <- c("ISOGG Haplogroup", "Count")
  kable(hg_table, caption="Haplogroup abaaa3a ISOGG classification")
}
```

```{r abaaa3a-dend, echo=FALSE, fig.cap="Dendrogram of haplogroup abaaa3a", fig.width=8, fig.height=5, out.width="100%", fig.pos="H"}
if("abaaa3a" %in% names(HapTable) && !is.null(HapTable[["abaaa3a"]])) {
  h=HapTable[["abaaa3a"]]
  if(nrow(h) >= 2) {
    h=h[order(h$ori),]
    L=dim(h)[2]
    ...
    labels_colors(dend) <- label_colors[order.dendrogram(dend)]
    par(mar = c(4, 4, 3, 2), xpd = FALSE)
    plot(dend, main = "Haplogroup abaaa3a", cex=.6, ylab="Number of differing SNPs", horiz=FALSE)
    par(xpd = TRUE)
    legend("topright", inset = c(0, -0.15), legend = group_levels, fill = colors[1:length(group_levels)], title = "Population", cex=.6, bty="n", ncol=min(3, length(group_levels)))
    par(mar = c(5, 4, 4, 2) + 0.1, xpd = FALSE)
  }
...
Terminal haplogroup **abba** demonstrates another African lineage:

```{r abba-table, echo=FALSE}
if("abba" %in% names(profs) && !is.null(profs[["abba"]])) {
  hg_table <- as.data.frame(table(profs[["abba"]]$HG))
  colnames(hg_table) <- c("ISOGG Haplogroup", "Count")
  kable(hg_table, caption="Haplogroup abba ISOGG classification")
}
```

```{r abba-dend, echo=FALSE, fig.cap="Dendrogram of haplogroup abba", fig.width=8, fig.height=5, out.width="100%", fig.pos="H"}
if("abba" %in% names(HapTable) && !is.null(HapTable[["abba"]])) {
  h=HapTable[["abba"]]
  if(nrow(h) >= 2) {
    h=h[order(h$ori),]
    L=dim(h)[2]
    ...
    dend <- as.dendrogram(cl)
    dend=dend%>%set("labels_cex",.7)
    dend=set_labels(dend,labels=profs[["abba"]]$HG)
    grouping_factor <- factor(as.character(h$ori))
    group_levels <- levels(grouping_factor)
    colors <- brewer.pal(max(3, length(group_levels)), "Set1")
    label_colors <- colors[as.numeric(grouping_factor)]
    labels_colors(dend) <- label_colors[order.dendrogram(dend)]
    par(mar = c(4, 4, 3, 2), xpd = FALSE)
    plot(dend, main = "Haplogroup abba", cex=.6, ylab="Number of differing SNPs", horiz=FALSE)
    par(xpd = TRUE)
    legend("topright", inset = c(0, -0.15), legend = group_levels, fill = colors[1:length(group_levels)], title = "Population", cex=.6, bty="n", ncol=min(3, length(group_levels)))
    par(mar = c(5, 4, 4, 2) + 0.1, xpd = FALSE)
  }
...
Traditional Y-chromosome phylogenetic approaches typically proceed through marker discovery, followed by tree construction using parsimony, maximum likelihood, or Bayesian methods [@jobling2004human; @poznik2016punctuated; @wei2013whole]. These methods require a priori identification of informative SNPs and assumptions about mutation models and branching processes. Recent comprehensive resequencing efforts have greatly expanded the Y-chromosome phylogeny [@hallast2015great; @batini2015large], providing robust frameworks against which data-driven approaches can be validated. In contrast, the yyhap framework directly exploits the linkage structure inherent in non-recombining regions to identify nested haplotype sets that automatically define phylogenetic relationships.
...
The identification rate of 23 errors among 8,008 SNPs × 1,223 individuals = 9.87 million genotype calls yields an estimated error rate of 0.00023% (2.3 × 10⁻⁶), substantially lower than typical sequencing error rates. This suggests that: (1) the 1000 Genomes Project Phase 3 data exhibit excellent quality, (2) many errors may have been filtered during our initial SNP selection, or (3) some true biological phenomena (rare mutations, gene conversion) are being misclassified as errors.
...
**Figure 4.** Subtree of branch **b** showing Eurasian haplogroup diversity, including European (R1a, R1b, I, J) and Native American (Q) lineages.

**Figure 5.** Subtree of branch **c** showing the structure of East Asian lineages. Haplogroups N and O are represented, with notable Finnish presence in subbranch c1b.
