...
Terminal haplogroup **abba1** demonstrates another African lineage:

```{r abba1-table, echo=FALSE}
if("abba1" %in% names(profs) && !is.null(profs[["abba1"]])) {
  hg_table <- as.data.frame(table(profs[["abba1"]]$HG))
  colnames(hg_table) <- c("ISOGG Haplogroup", "Count")
  kable(hg_table, caption="Haplogroup abba1 ISOGG classification")
}
```

```{r abba1-dend, echo=FALSE, fig.cap="Dendrogram of haplogroup abba1", fig.width=8, fig.height=5, out.width="100%", fig.pos="H"}
if("abba1" %in% names(HapTable) && !is.null(HapTable[["abba1"]])) {
  h=HapTable[["abba1"]]
  if(nrow(h) >= 2) {
    h=h[order(h$ori),]
    L=dim(h)[2]
    ...
    labels_colors(dend) <- label_colors[order.dendrogram(dend)]
    par(mar = c(4, 4, 3, 2), xpd = FALSE)
    plot(dend, main = "Haplogroup abba1", cex=.6, ylab="Number of differing SNPs", horiz=FALSE)
    par(xpd = TRUE)
    legend("topright", inset = c(0, -0.15), legend = group_levels, fill = colors[1:length(group_levels)], title = "Population", cex=.6, bty="n", ncol=min(3, length(group_levels)))
    par(mar = c(5, 4, 4, 2) + 0.1, xpd = FALSE)
  }
